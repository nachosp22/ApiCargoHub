...
$output = Join-Path $PSScriptRoot 'TFG_CargoHub_tablas_clases_limpio.docx'
...
function FixedTable($headers, $widths, $rows) {
...
foreach ($c in $classes) {
  $body += H2 $c.Name
  $rows = @()
  $rows += ,@('Informacion', '', 'Tabla JPA', $c.Table)
  $rows += ,@('Informacion', '', 'Responsabilidad', $c.Resp)
  foreach ($a in $c.Attr) { $p = $a -split ': ',2; $rows += ,@('Atributo','- privado',$p[0], $(if($p.Count -gt 1){$p[1]}else{''})) }
  foreach ($m in $c.Meth) { $p = $m -split ': ',2; $rows += ,@('Operacion','+ publico',$p[0], $(if($p.Count -gt 1){$p[1]}else{'Entidad + controller/service'})) }
  foreach ($r in $c.Rel) { $p = $r -split ': ',2; $rows += ,@('Relacion','', $p[0], $(if($p.Count -gt 1){$p[1]}else{''})) }
  $body += FixedTable @('Tipo','Visibilidad','Nombre','Detalle') @(2200,2200,4200,6400) $rows
}

$body += H2 'Enumeraciones'
$body += FixedTable @('Enumeracion','Valores') @(4200,10800) @(

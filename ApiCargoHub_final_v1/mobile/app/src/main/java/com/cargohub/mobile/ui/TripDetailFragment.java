...
public class TripDetailFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Solo permitir pull-to-refresh cuando el scroll está arriba del todo
        View scrollView = view.findViewById(R.id.tripDetailScrollView);
        if (scrollView != null) {
    ...
    private void renderActions(@NonNull Porte porte) {
        ...
        if (actions.isEmpty()) {
            nextActionHintText.setVisibility(View.GONE);
    ...
    private void bindAction(@NonNull MaterialButton button, @NonNull PorteRepository.PorteAction action) {
        ...
        if (action == PorteRepository.PorteAction.ACCEPT_OFFER) {
            button.setBackgroundTintList(android.content.res.ColorStateList.valueOf(
                    androidx.core.content.ContextCompat.getColor(requireContext(), R.color.ch_success)));
            button.setTextColor(androidx.core.content.ContextCompat.getColor(requireContext(), R.color.white));
        } else if (action == PorteRepository.PorteAction.REJECT_OFFER) {
            button.setBackgroundTintList(android.content.res.ColorStateList.valueOf(
                    androidx.core.content.ContextCompat.getColor(requireContext(), R.color.ch_error)));
            button.setTextColor(androidx.core.content.ContextCompat.getColor(requireContext(), R.color.white));
        }
    }
    ...
    private void executeAction(@NonNull PorteRepository.PorteAction action) {
        ...
        if (action == PorteRepository.PorteAction.ACCEPT_OFFER) {
            ...
            porteRepository.acceptOffer(currentPorte.getId(), conductorId, result -> handleTripMutationResult(result, true));
            return;
        }
        ...
        if (action == PorteRepository.PorteAction.START_TRIP) {
            EstadoPorte currentState = currentPorte.getEstadoPorte();
            if (currentState != EstadoPorte.ASIGNADO) return;
            setActionsEnabled(false);
            porteRepository.changeTripState(currentPorte.getId(), currentState, EstadoPorte.EN_RECOGIDA, result -> {
                if (!isAdded()) return;
                ...
                if (result.isSuccessful()) {
                    ...
                    if (msg == null || msg.isEmpty()) {
                        msg = getString(R.string.generic_api_error_short);
                    }
                    View v = getView();
                    if (isAdded() && v != null) {
        ...
        setActionsEnabled(false);
        porteRepository.changeTripState(currentPorte.getId(), currentState, EstadoPorte.ENTREGADO, this::handleTripMutationResult);
    }

    private void handleTripMutationResult(@NonNull RepositoryResult<Porte> result) {
        handleTripMutationResult(result, false);
    }

    private void handleTripMutationResult(@NonNull RepositoryResult<Porte> result, boolean wasOfferAccept) {
        if (!isAdded()) {
            return;
        }
        View view = getView();
        ...
        renderTrip(currentPorte);
        if (wasOfferAccept) {
            notifyTripListChanged();
            Snackbar.make(view, R.string.offer_accept_success, Snackbar.LENGTH_LONG).show();
            view.postDelayed(() -> {
                if (isAdded()) {
    ...
    private void notifyTripListChanged() {
        getParentFragmentManager().setFragmentResult(TripListFragment.REQUEST_KEY_TRIP_MUTATED, new Bundle());
    }
    ...
    private boolean isTrackable(@Nullable Porte porte) {
        ...
        return state == EstadoPorte.EN_RECOGIDA || state == EstadoPorte.EN_TRANSITO;
    }

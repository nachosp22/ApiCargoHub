...
$output = Join-Path $PSScriptRoot 'TFG_CargoHub_tablas_clases_vertical.docx'
...
foreach ($c in $classes) {
  ...
  $body += FixedTable $c.Name @('Tipo','Elemento','Detalle') @(1800,3300,4400) $rows
  $body += P ''
}

$body += FixedTable 'Enumeraciones' @('Enumeracion','Valores') @(2800,6700) @(
